...
class RequestsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private var savedVotesTime: Long = -1
    ...
    fun loadApps(debug: Boolean = false) {
        viewModelScope.launch {
            ...
            requestVotes(appsToRequest)
            appsToRequestData.postValue(appsToRequest)
        }
    }

    private fun getVotesRequestTimeLimit(): Long {
        val timeLimitSeconds = context.integer(R.integer.votes_request_limit_in_seconds)
        return TimeUnit.MINUTES.toSeconds(timeLimitSeconds.toLong())
    }

    @SuppressLint("SimpleDateFormat")
    private fun getVotesRequestTimeLeft(): Long {
        if (savedVotesTime < 0) return -1
        val elapsedTime = System.currentTimeMillis() - savedVotesTime
        return getVotesRequestTimeLimit() - elapsedTime - 500
    }

    private suspend fun requestVotes(apps: ArrayList<RequestApp>) {
        val timeLeft = getVotesRequestTimeLeft()
        if (apps.isEmpty() || timeLeft > 0) return
        savedVotesTime = System.currentTimeMillis()
        return SendIconRequest.sendIconVotesRequest(context, apps) { result ->
            for (component in result.result) {
                apps.forEach {
                    if (it.component == component.componentInfo)
                        it.votes = component.votes
                }
            }
        }
    }
